...
pub fn default_config(bootstrap: &ConfigBootstrap) -> Result<Config, config::ConfigError> {
    let mut cfg = Config::new();

    // Common settings
    cfg.set_default("common.message_cache_size", 10)?;
    cfg.set_default("common.message_cache_ttl", 1440)?;
    cfg.set_default("common.peer_allowlist", Vec::<String>::new())?;
    cfg.set_default("common.rpc_max_simultaneous_sessions", 1000)?;
    cfg.set_default("common.liveness_max_sessions", 0)?;
    cfg.set_default("common.denylist_ban_period", 1440)?;
    cfg.set_default("common.buffer_size_base_node", 1_500)?;
    cfg.set_default("common.buffer_size_console_wallet", 50_000)?;
    cfg.set_default("common.buffer_rate_limit_base_node", 1_000)?;
    cfg.set_default("common.buffer_rate_limit_console_wallet", 1_000)?;
    cfg.set_default("common.dedup_cache_capacity", 2_500)?;
    cfg.set_default("common.fetch_blocks_timeout", 150)?;
    cfg.set_default("common.fetch_utxos_timeout", 600)?;
    cfg.set_default("common.service_request_timeout", 180)?;

    // Wallet settings
    cfg.set_default("wallet.grpc_enabled", false)?;
    cfg.set_default("wallet.grpc_address", "127.0.0.1:18043")?;
    cfg.set_default(
    ...
    cfg.set_default("wallet.base_node_query_timeout", 60)?;
    cfg.set_default("wallet.base_node_service_refresh_interval", 5)?;
    cfg.set_default("wallet.base_node_service_request_max_age", 60)?;
    cfg.set_default("wallet.balance_enquiry_cooldown_period", 1)?;
    ...
    cfg.set_default("wallet.transaction_direct_send_timeout", 20)?;
    cfg.set_default("wallet.transaction_broadcast_send_timeout", 60)?;
    cfg.set_default("wallet.prevent_fee_gt_amount", true)?;
    cfg.set_default("wallet.transaction_routing_mechanism", "DirectAndStoreAndForward")?;
    cfg.set_default("wallet.command_send_wait_stage", "Broadcast")?;
    cfg.set_default("wallet.command_send_wait_timeout", 300)?;
    cfg.set_default("wallet.base_node_service_peers", Vec::<String>::new())?;

    //---------------------------------- Mainnet Defaults --------------------------------------------//

    cfg.set_default("common.network", "mainnet")?;

    // Mainnet base node defaults
    cfg.set_default("base_node.mainnet.db_type", "lmdb")?;
    ...
    cfg.set_default("base_node.mainnet.pruning_horizon", 0)?;
    ...
    cfg.set_default("base_node.mainnet.grpc_enabled", false)?;
    ...
    cfg.set_default("wallet.grpc_address", "127.0.0.1:18143")?;
    cfg.set_default("base_node.mainnet.flood_ban_max_msg_count", 100_000)?;

    //---------------------------------- Weatherwax Defaults --------------------------------------------//

    cfg.set_default("base_node.weatherwax.db_type", "lmdb")?;
    ...
    cfg.set_default("base_node.weatherwax.pruning_horizon", 0)?;
    ...
    cfg.set_default("base_node.weatherwax.grpc_enabled", false)?;
    cfg.set_default("base_node.weatherwax.grpc_base_node_address", "127.0.0.1:18142")?;

    //---------------------------------- Igor Defaults --------------------------------------------//

    cfg.set_default("base_node.igor.db_type", "lmdb")?;
    cfg.set_default("base_node.igor.orphan_storage_capacity", 720)?;
    cfg.set_default("base_node.igor.orphan_db_clean_out_threshold", 0)?;
    cfg.set_default("base_node.igor.pruning_horizon", 0)?;
    ...
    cfg.set_default("base_node.igor.grpc_enabled", false)?;
...
fn set_common_network_defaults(cfg: &mut Config) -> Result<(), config::ConfigError> {
    for network in ["mainnet", "dibbler", "igor", "localnet"] {
        ...
        cfg.set_default(&key, vec!["versions.tari.com"])?;
...
fn set_stratum_transcoder_defaults(cfg: &mut Config) -> Result<(), config::ConfigError> {
...
fn set_merge_mining_defaults(cfg: &mut Config) -> Result<(), config::ConfigError> {
    //---------------------------------- common defaults --------------------------------------------//
    ...
    cfg.set_default("merge_mining_proxy.igor.monerod_username", "")?;
    cfg.set_default("merge_mining_proxy.igor.monerod_password", "")?;
...
fn set_mining_node_defaults(cfg: &mut Config) -> Result<(), config::ConfigError> {
    cfg.set_default("mining_node.num_mining_threads", 1)?;
    cfg.set_default("mining_node.mine_on_tip_only", true)?;
    cfg.set_default("mining_node.validate_tip_timeout_sec", 0)?;
    Ok(())
}
